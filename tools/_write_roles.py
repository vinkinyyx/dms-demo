...
p=Path('frontend-vue/src/views/Roles.vue')
...
function parentCodeOf(code, type) {
  if (type === 'button') return code.split(':').slice(0, 2).join(':') + ':view'
  if (type === 'api') return code.split(':').slice(0, 2).join(':') + ':view'
  return null
}
...
<style scoped>
.roles-page { background: #fff; border-radius: 4px; padding: 16px; box-shadow: 0 1px 3px rgb(0 0 0 / .1); }
...
.perm-tree { max-height: 560px; overflow: auto; border: 1px solid #ebeef5; border-radius: 4px; padding: 8px; }
</style>
