    ...
    <el-card shadow="never" class="rd-logs">
      ...
      <el-timeline v-if="logs.length">
        <el-timeline-item v-for="(log, idx) in logs" :key="idx" :timestamp="fmt(log.atTime)" placement="top">
          ...
          <div class="log-changes" v-if="log.changes">{{ enhanceChanges(log.changes, { ...(detail || {}), productMap: lookupCache.products }) }}</div>
        </el-timeline-item>
      </el-timeline>
...
import { getOperationLogs, httpGet } from '@/api/crud'
import { MODULE_CONFIGS } from '@/config/modules'
import { statusText as st, statusTagType as stt, fmt as dictFmt, actionText, enhanceChanges } from '@/utils/dict'
...
function pickerName(row, k) {
  const stem = k.replace(/Id$/, '')
  const nameKeys = [`${stem}Name`, 'partnerName', 'dealerName', 'regionName', 'categoryName', 'productLineName', 'productName', 'warehouseName', 'hospitalName', 'supplierName', 'orgName', 'roleName']
  for (const nk of nameKeys) {
    if (row[nk] != null && row[nk] !== '') return row[nk]
  }
  const codeKeys = [`${stem}Code`, 'productCode', 'dealerCode', 'regionCode', 'warehouseCode']
  for (const ck of codeKeys) {
    if (row[ck]) return row[ck]
  }
  return null
}
...
const lookupCache = {}
async function ensureLookup(resource) {
  if (!resource || !LOOKUP_ENDPOINTS[resource]) return
  if (lookupCache[resource]) return
  lookupCache[resource] = (async () => {
    try {
      const ep = LOOKUP_ENDPOINTS[resource]
      const res = await httpGet(ep.url, { size: 500 })
      const list = res?.data?.list || res?.data?.records || res?.data || []
      const map = new Map()
      for (const x of list) {
        const v = x.value != null ? x.value : x.id
        const label = x.label || x[ep.label] || x.name || x.code || (v != null ? String(v) : '')
        if (v != null) map.set(String(v), { label, code: x.code || '' })
      }
      return map
    } catch (e) {
      return new Map()
  ...
  lookupCache[resource] = await lookupCache[resource]
}
function pickerResourceOf(f) {
  if (!f) return null
  if (f.picker && typeof f.picker === 'object') return f.picker.resource
  return f.picker || null
}
const pendingLookups = new Set()
function triggerLookup(resource) {
  if (!resource || pendingLookups.has(resource)) return
  pendingLookups.add(resource)
  ensureLookup(resource).then(() => {
    if (detail.value) detail.value = { ...detail.value }
    if (Array.isArray(logs.value) && logs.value.length) logs.value = [...logs.value]
  })
}
function resolveLookup(f, id) {
  const resource = pickerResourceOf(f)
  const map = resource ? lookupCache[resource] : null
  if (map instanceof Map && id != null && id !== '') {
    const hit = map.get(String(id))
    if (hit) return hit.code ? `${hit.code} ${hit.label}` : hit.label
  }
  return null
}
function fieldDisplay(f, row) {
  const k = f.key
  if (k === 'productId') return [row.productCode, row.productName].filter(Boolean).join(' ') || row.sku || resolveLookup(f, row[k]) || display(f, row[k])
  if (k === 'partnerId') {
    if ((row[k] == null || row[k] === 0 || row[k] === '0') && !row.partnerName && !row.dealerName && !row.supplierName) return '全部经销商（通用价）'
    return row.partnerName || row.dealerName || row.supplierName || resolveLookup(f, row[k]) || display(f, row[k])
  }
  if (k === 'bomParentProductId') return [row.bomParentCode, row.bomParentName].filter(Boolean).join(' ') || display(f, row[k])
  if (k === 'dealerId') return [row.dealerCode, row.dealerName].filter(Boolean).join(' ') || pickerName(row, k) || resolveLookup(f, row[k]) || display(f, row[k])
  if (k === 'regionId') return row.regionName || row.regionCode || resolveLookup(f, row[k]) || display(f, row[k])
  if (k === 'categoryId') return [row.categoryCode, row.categoryName].filter(Boolean).join(' ') || resolveLookup(f, row[k]) || display(f, row[k])
  if (k === 'productLineId') return [row.productLineCode, row.productLineName].filter(Boolean).join(' ') || resolveLookup(f, row[k]) || display(f, row[k])
  if (k === 'warehouseId') return [row.warehouseCode, row.warehouseName].filter(Boolean).join(' ') || resolveLookup(f, row[k]) || display(f, row[k])
  if (k === 'hospitalId') return [row.hospitalCode, row.hospitalName].filter(Boolean).join(' ') || resolveLookup(f, row[k]) || display(f, row[k])
  if (k === 'supplierId') return [row.supplierCode, row.supplierName].filter(Boolean).join(' ') || resolveLookup(f, row[k]) || display(f, row[k])
  if (k === 'inclPrice' && row.priceContext === 'BOM_HEADER') return '见子件价格'
  if (k === 'exclPrice' && row.priceContext === 'BOM_HEADER') return '见子件价格'
  const raw = row[k]
  if (f.type === 'picker' || /Id$/.test(k)) {
    const name = pickerName(row, k)
    if (name) return name
    const looked = resolveLookup(f, raw)
    if (looked) return looked
  }
  const translated = dictFmt(raw, k)
  if (translated !== String(raw == null ? '' : raw)) return translated
...
async function load() {
  ...
  try {
    ...
    if (detail.value) {
      for (const f of fields.value) {
        ...
        const stem = (f.key || '').replace(/Id$/, '')
        const hasName = detail.value[`${stem}Name`] || detail.value.partnerName || detail.value.dealerName
        if ((v != null && v !== '' && v !== 0) || (f.key === 'partnerId' && v === 0 && !hasName)) {
          triggerLookup(resource)
        }
      }
    }
  ...
  try {
    ...
    if (logs.value.some(x => x && typeof x.changes === 'string' && /product\s*=\s*\d+/i.test(x.changes))) {
      triggerLookup('products')
    }
  } catch (e) { logs.value = [] }
}
