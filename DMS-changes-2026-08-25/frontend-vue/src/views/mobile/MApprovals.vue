...
async function loadMore(){ if(loading.value||refreshing.value)return; loading.value=true; try{ const params={page:page.value,size:size.value}; const res=active.value==='todo'?await myTodoTasks(params):active.value==='done'?await myDoneTasks(params):active.value==='submitted'?await mySubmitted(params):await myCc(params); const d=res.data||{}; const list=d.list||[]; rows.value.push(...list); total.value=d.total||0; if(rows.value.length>=total.value||!list.length)finished.value=true; page.value++ }catch(e){ finished.value=true }finally{ loading.value=false; refreshing.value=false } }
