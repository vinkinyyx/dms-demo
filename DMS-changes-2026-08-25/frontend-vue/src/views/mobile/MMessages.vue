...
async function loadMore(){ if(loading.value||refreshing.value)return; loading.value=true; try{ const params={page:page.value,size:size.value}; if(active.value==='unread')params.isRead=false; else if(active.value)params.refType=active.value; const r=await request({url:'/api/notifications',method:'get',params}); const d=r.data||{}; const list=d.list||[]; rows.value.push(...list); total.value=d.total||0; if(rows.value.length>=total.value||!list.length)finished.value=true; page.value++ }catch(e){ finished.value=true }finally{ loading.value=false; refreshing.value=false } }
