...
public class BizDocListController {
    ...
    public ApiResponse<Map<String, Object>> listSalesOuts(
            ...
            @RequestParam(required = false) String sourceOrderCode,
            @RequestParam(required = false) String salesDateFrom,
            @RequestParam(required = false) String salesDateTo) {
        ...
        if (code != null && !code.isBlank()) { where.append(" AND so.code ILIKE ?").append(idx++); params.add("%" + code + "%"); }
        if (warehouseId != null) { where.append(" AND so.warehouse_id = ?").append(idx++); params.add(warehouseId); }
        if (terminalId != null) { where.append(" AND so.terminal_id = ?").append(idx++); params.add(terminalId); }
        if (dealerName != null && !dealerName.isBlank()) { where.append(" AND d.name ILIKE ?").append(idx++); params.add("%" + dealerName + "%"); }
        if (warehouseName != null && !warehouseName.isBlank()) { where.append(" AND w.name ILIKE ?").append(idx++); params.add("%" + warehouseName + "%"); }
        if (sourceOrderCode != null && !sourceOrderCode.isBlank()) { where.append(" AND o.code ILIKE ?").append(idx++); params.add("%" + sourceOrderCode + "%"); }
        if (salesDateFrom != null && !salesDateFrom.isBlank()) { java.sql.Timestamp __t = com.dms.common.util.SpecUtil.rangeBound(salesDateFrom, true); if (__t != null) { where.append(" AND so.sales_date >= ?").append(idx++); params.add(__t); } }
        if (salesDateTo != null && !salesDateTo.isBlank()) { java.sql.Timestamp __t = com.dms.common.util.SpecUtil.rangeBound(salesDateTo, false); if (__t != null) { where.append(com.dms.common.util.SpecUtil.hasTime(salesDateTo) ? " AND so.sales_date <= ?" : " AND so.sales_date < ?").append(idx++); params.add(__t); } }

        String cntFrom = "FROM sales_outs so " +
                "LEFT JOIN dealers d ON d.id = so.dealer_id " +
                "LEFT JOIN warehouses w ON w.id = so.warehouse_id " +
                "LEFT JOIN orders o ON o.id = so.source_order_id ";
        var cntQ = em.createNativeQuery("SELECT COUNT(*) " + cntFrom + where);
    ...
    public ApiResponse<Map<String, Object>> listReceipts(
            ...
            @RequestParam(required = false) String code,
            @RequestParam(required = false) String warehouseName,
            @RequestParam(required = false) String sourcePoCode,
            @RequestParam(required = false) String receiptDateFrom,
            @RequestParam(required = false) String receiptDateTo) {
        UUID tid = TenantContext.getTenantId();
        int safePage = PagingUtil.normalizePage(page); int safeSize = PagingUtil.normalizeSize(size); int offset = (safePage - 1) * safeSize;
        ...
        if (code != null && !code.isBlank()) { where.append(" AND r.code ILIKE ?").append(idx++); params.add("%" + code + "%"); }
        if (warehouseName != null && !warehouseName.isBlank()) { where.append(" AND w.name ILIKE ?").append(idx++); params.add("%" + warehouseName + "%"); }
        if (sourcePoCode != null && !sourcePoCode.isBlank()) { where.append(" AND po.code ILIKE ?").append(idx++); params.add("%" + sourcePoCode + "%"); }
        if (receiptDateFrom != null && !receiptDateFrom.isBlank()) { java.sql.Timestamp __t = com.dms.common.util.SpecUtil.rangeBound(receiptDateFrom, true); if (__t != null) { where.append(" AND r.received_at >= ?").append(idx++); params.add(__t); } }
        if (receiptDateTo != null && !receiptDateTo.isBlank()) { java.sql.Timestamp __t = com.dms.common.util.SpecUtil.rangeBound(receiptDateTo, false); if (__t != null) { where.append(com.dms.common.util.SpecUtil.hasTime(receiptDateTo) ? " AND r.received_at <= ?" : " AND r.received_at < ?").append(idx++); params.add(__t); } }

        String cntFrom = "FROM receipts r " +
