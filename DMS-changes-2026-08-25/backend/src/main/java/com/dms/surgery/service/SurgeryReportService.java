package com.dms.surgery.service;
...
public class SurgeryReportService {
    ...
    public ApiResponse<Map<String, Object>> list(
            ...
            String dealerName,
            String terminalName,
            String surgeryDateFrom,
            String surgeryDateTo,
            String doctorName,
        ...
        StringBuilder where = new StringBuilder("WHERE sr.tenant_id = ?1 AND sr.deleted_at IS NULL");
        List<Object> params = new ArrayList<>();
        params.add(tid);
        int idx = 2;
        if (allowed != null) {
            where.append(" AND sr.dealer_id = ANY(?").append(idx++).append(")");
            params.add(allowed.toArray(new Long[0]));
        }
        if (id != null) {
            where.append(" AND sr.id = ?").append(idx++);
            params.add(id);
        }
        if (code != null && !code.isBlank()) {
            where.append(" AND sr.code ILIKE ?").append(idx++);
            params.add("%" + code.trim() + "%");
        }
        if (dealerName != null && !dealerName.isBlank()) {
            where.append(" AND d.name ILIKE ?").append(idx++);
            params.add("%" + dealerName.trim() + "%");
        }
        if (terminalName != null && !terminalName.isBlank()) {
            where.append(" AND h.name ILIKE ?").append(idx++);
            params.add("%" + terminalName.trim() + "%");
        }
        if (surgeryDateFrom != null && !surgeryDateFrom.isBlank()) {
            try {
                java.sql.Date d = java.sql.Date.valueOf(LocalDate.parse(surgeryDateFrom.trim()));
                where.append(" AND sr.surgery_date >= ?").append(idx++);
                params.add(d);
            } catch (Exception ignored) { }
        }
        if (surgeryDateTo != null && !surgeryDateTo.isBlank()) {
            try {
                LocalDate toDate = LocalDate.parse(surgeryDateTo.trim());
                where.append(" AND sr.surgery_date < ?").append(idx++);
                params.add(java.sql.Date.valueOf(toDate.plusDays(1)));
            } catch (Exception ignored) { }
        }
        if (doctorName != null && !doctorName.isBlank()) {
            where.append(" AND sr.doctor_name ILIKE ?").append(idx++);
            params.add("%" + doctorName.trim() + "%");
        }
        if (attachmentName != null && !attachmentName.isBlank()) {
            where.append(" AND sr.attachment_name ILIKE ?").append(idx++);
            params.add("%" + attachmentName.trim() + "%");
        }
        if (status != null && !status.isBlank()) {
            where.append(" AND sr.status = ?").append(idx++);
        ...
        if (createdAtFrom != null && !createdAtFrom.isBlank()) {
            ...
            if (t != null) {
                where.append(" AND sr.created_at >= ?").append(idx++);
                params.add(t);
            }
        }
        if (createdAtTo != null && !createdAtTo.isBlank()) {
            ...
            if (t != null) {
                if (com.dms.common.util.SpecUtil.hasTime(createdAtTo)) {
                    where.append(" AND sr.created_at <= ?").append(idx++);
                } else {
                    where.append(" AND sr.created_at < ?").append(idx++);
                }
        ...
        if (updatedAtFrom != null && !updatedAtFrom.isBlank()) {
            java.sql.Timestamp t = com.dms.common.util.SpecUtil.rangeBound(updatedAtFrom, true);
            if (t != null) {
                where.append(" AND sr.updated_at >= ?").append(idx++);
                params.add(t);
            }
        }
        if (updatedAtTo != null && !updatedAtTo.isBlank()) {
            java.sql.Timestamp t = com.dms.common.util.SpecUtil.rangeBound(updatedAtTo, false);
            if (t != null) {
                if (com.dms.common.util.SpecUtil.hasTime(updatedAtTo)) {
                    where.append(" AND sr.updated_at <= ?").append(idx++);
                } else {
                    where.append(" AND sr.updated_at < ?").append(idx++);
                }
        ...
        String joinClause = " FROM surgery_reports sr " +
                "LEFT JOIN dealers d ON d.id = sr.dealer_id " +
                "LEFT JOIN hospitals h ON h.id = sr.terminal_id ";

        String cntSql = "SELECT COUNT(*) " + joinClause + where;
        ...
        String listSql = "SELECT sr.id, sr.code, sr.dealer_id, sr.terminal_id, sr.sales_user_id, " +
                ...
                joinClause + where +
                " ORDER BY sr.updated_at DESC NULLS LAST, sr.id DESC LIMIT ?" + idx + " OFFSET ?" + (idx + 1);
        var lq = em.createNativeQuery(listSql, Tuple.class);
        for (int i = 0; i < params.size(); i++) lq.setParameter(i + 1, params.get(i));
        lq.setParameter(idx, safeSize);
        lq.setParameter(idx + 1, offset);
...
private boolean isSerialManaged(UUID tid, Long productId) {
    ...
    public ApiResponse<Map<String, Object>> getDetail(Long id) {
        UUID tid = TenantContext.getTenantId();
        @SuppressWarnings("unchecked")
