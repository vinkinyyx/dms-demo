...
public class SalesPositionService {
    ...
    public ApiResponse<Map<String, Object>> list(
        ...
        if (page < 1) throw new BusinessException(ErrorCode.PARAM_INVALID, "页码从 1 起");
        if (size < 1) throw new BusinessException(ErrorCode.PARAM_INVALID, "每页条数至少为 1");
        int offset = (page - 1) * size;

        StringBuilder where = new StringBuilder(" WHERE sp.tenant_id = ?1 AND sp.deleted_at IS NULL");
        List<Object> params = new ArrayList<>();
        params.add(tid);
        int idx = 2;
        if (id != null) {
            where.append(" AND sp.id = ?").append(idx++);
            params.add(id);
        }
        if (code != null && !code.isBlank()) {
            where.append(" AND sp.code ILIKE ?").append(idx++);
            params.add("%" + code.trim() + "%");
        }
        if (name != null && !name.isBlank()) {
            where.append(" AND sp.name ILIKE ?").append(idx++);
            params.add("%" + name.trim() + "%");
        }
        if (level != null && !level.isBlank()) {
            where.append(" AND CAST(sp.level AS VARCHAR) ILIKE ?").append(idx++);
            params.add("%" + level.trim() + "%");
        }
        if (status != null && !status.isBlank()) {
            where.append(" AND sp.status = ?").append(idx++);
        ...
        if (createdAtFrom != null && !createdAtFrom.isBlank()) {
            ...
            if (__t != null) { where.append(" AND sp.created_at >= ?").append(idx++); params.add(__t); }
        }
        if (createdAtTo != null && !createdAtTo.isBlank()) {
            java.sql.Timestamp __t = com.dms.common.util.SpecUtil.rangeBound(createdAtTo, false);
            if (__t != null) {
                where.append(com.dms.common.util.SpecUtil.hasTime(createdAtTo) ? " AND sp.created_at <= ?" : " AND sp.created_at < ?").append(idx++);
                params.add(__t);
            }
        }
        if (updatedAtFrom != null && !updatedAtFrom.isBlank()) {
            ...
            if (__t != null) { where.append(" AND sp.updated_at >= ?").append(idx++); params.add(__t); }
        }
        if (updatedAtTo != null && !updatedAtTo.isBlank()) {
            java.sql.Timestamp __t = com.dms.common.util.SpecUtil.rangeBound(updatedAtTo, false);
            if (__t != null) {
                where.append(com.dms.common.util.SpecUtil.hasTime(updatedAtTo) ? " AND sp.updated_at <= ?" : " AND sp.updated_at < ?").append(idx++);
                params.add(__t);
            }
        }

        var cnt = em.createNativeQuery("SELECT COUNT(*) FROM sales_positions sp" + where);
        ...
        var q = em.createNativeQuery(
                ...
                "FROM sales_positions sp" + where +
                " ORDER BY sp.sort_order, sp.level, sp.id LIMIT ?" + idx + " OFFSET ?" + (idx + 1), Tuple.class);
        for (int i = 0; i < params.size(); i++) q.setParameter(i + 1, params.get(i));
        q.setParameter(idx, size).setParameter(idx + 1, offset);
    ...
    public ApiResponse<Map<String, Object>> getOne(Long id) {
        var q = em.createNativeQuery(
                "SELECT sp.id, sp.code, sp.name, sp.level, sp.parent_id, sp.region, sp.status, sp.sort_order, "
     ...
     */
    @Transactional
    public ApiResponse<Map<String, Object>> bindUsers(Long id, Map<String, Object> body) {
        UUID tid = TenantContext.getTenantId();
        @SuppressWarnings("unchecked")
     ...
     */
    @Transactional
    public ApiResponse<Map<String, Object>> bindDealers(Long id, Map<String, Object> body) {
        UUID tid = TenantContext.getTenantId();
        @SuppressWarnings("unchecked")
     ...
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW, readOnly = true)
    public ApiResponse<List<Map<String, Object>>> candidateUsersForPosition(Long id) {
        UUID tid = TenantContext.getTenantId();
        @SuppressWarnings("unchecked")
     ...
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW, readOnly = true)
    public ApiResponse<List<Map<String, Object>>> candidateDealersForPosition(Long id) {
        UUID tid = TenantContext.getTenantId();
        @SuppressWarnings("unchecked")
        List<Tuple> rows = em.createNativeQuery(
     ...
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW, readOnly = true)
    public ApiResponse<List<Map<String, Object>>> getPositionUsers(Long id) {
        UUID tid = TenantContext.getTenantId();
        @SuppressWarnings("unchecked")
        List<Tuple> rows = em.createNativeQuery(
     ...
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW, readOnly = true)
    public ApiResponse<List<Map<String, Object>>> getPositionDealers(Long id) {
        UUID tid = TenantContext.getTenantId();
        @SuppressWarnings("unchecked")
        List<Tuple> rows = em.createNativeQuery(
