...
- `/dms/`：业务前端 PC 工作台。
- `/dms/mobile/`：业务前端移动端入口。
- `/dms/admin/`：平台后台。
- `/api/`、`/auth/`、`/actuator/`：反向代理到后端，不加 `/dms` 前缀。
- 生产根路径 `/` 为产品宣传手册；测试根路径为部署落地页/历史入口。
...
- 默认数据库：PostgreSQL，生产库名 `dms`，测试库名 `dms_test`。
- 测试库登录：`sudo docker exec -it dms-test-postgres psql -U dms -d dms_test`。
- 生产库登录：先在生产服务器执行 `c=$(docker ps --format '{{.Names}}' | grep 'dms-prod-postgres' | head -1)`，再执行 `docker exec -it "$c" psql -U dms -d dms`。
