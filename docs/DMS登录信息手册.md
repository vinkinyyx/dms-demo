...
**更新时间**: 2026-08-13  
**当前版本**: v3.11.0  
**适用范围**: 测试环境（43.128.145.141:8083；旧测试 8.133.193.238:8083/8082）与生产环境（8081/8080）
...
|---|---|---|---|---|
| 新测试 | `http://43.128.145.141:8083` | Nginx 同源 `/api` 反代内网 `8080` | `dms_test` | 最新 DMS 测试环境，Docker Compose 部署 |
| 旧测试 | `http://8.133.193.238:8083` | `http://8.133.193.238:8082` | `dms_test` | 历史测试环境 |
...
| 入口 | 地址 | 说明 |
|---|---|---|
| 业务前台登录页 | http://43.128.145.141:8083/login | PC 登录页，租户 `default`，账号 `sys_admin`，密码 `Dms@123456` |
| 移动端 H5 登录页 | http://43.128.145.141:8083/mobile/login | 手机浏览器访问，账号与 PC 业务前台相同 |
| 业务工作台 | http://43.128.145.141:8083/ | 最新测试 PC 工作台 |
| 平台后台 | http://43.128.145.141:8083/admin/ | 账号 `admin`，密码 `Sh123456` |
| 健康检查 | http://43.128.145.141:8083/actuator/health | 返回 `{"status":"UP"}` 表示正常 |
