...
> **版本**: v1.4
> **创建时间**: 2026-08-11
> **最近更新**: 2026-08-31（v4.5.3 PC 工作台 UI 增强：内容区顶部新增面包屑导航（Breadcrumb.vue，菜单路径「首页/分组/菜单/详情」）+ 多菜单页签 Tags-View（TagsBar.vue，点击切换/×关闭/中键关闭/右键菜单刷新·关闭·关闭其他·关闭全部，首页 affix 固定）；新增 pageMeta.js 元信息解析（FLAT_MENU 扁平化 + 27 条 DETAIL_PARENT 详情页正则映射）与 store/tags.js（sessionStorage key `dms:tags-view` 持久化，注意持久化值为 tag 数组本身）；layout/index.vue 挂两组件 + keep-alive :max=12（noCache 除外）+ reloading v-if 实现页签刷新重建；纯前端、无 Flyway/接口变更；本地 vitest 36/36 + build 通过，已部署测试环境（stamp 20260831-165944）并按铁律9 浏览器终验全过；新增踩坑点 58-61）
> 上一更新: 2026-08-31（v4.5.2 接入 GitHub Actions CI 流水线：.github/workflows/ci.yml 双 job（backend `mvn -B verify` / frontend `npm ci`+vitest+vite build），push/PR 到 main 触发；全量测试基线修复 12 例（11 例旧 Controller 集成测试 403 补 grantPermissions + 1 例 V4CalculatorTest PERCENT 折数语义造数 20→80），后端 147/147 全绿、前端 vitest 36 例 + build 通过；新增踩坑点 55-57；无 Flyway 变更）
> 上一更新: 2026-08-31（v4.5.1 跨租户协同自动化回归补齐 + 分批/幂等/幽灵库存 4+1 缺陷修复；v450_collab.py 56/56、CrossTenantCollabIntegrationTest 11/11；Flyway V141/V142；踩坑点 52-54）
> 上一更新: 2026-08-30（v4.5.0 MINOR 厂家进销存开关持久化 V140 + 厂家↔经销商跨租户订单协同 CrossTenantCollabService 路径A/B）
> 上一更新: 2026-08-30（v4.4.7 百分比折扣语义颠倒修复：手动行/整单折扣 PERCENT 误按“减免比例”(98→减98%)，改为中文“折数=实付比例”(98折→付98%/减2%)，V4Money.signedDiscount；移动端摘要显示 9.8 折）
> 上一更新: 2026-08-30（v4.4.6 移动端“很慢/刷不出来”修复：令牌刷新误打 /auth/refresh 应为 /api/auth/refresh、登录过期按设备跳转、op_log @PrePersist 截断；L73/L74）
> 上一更新: 2026-08-29（v4.4.5 智能下单 4 项反馈：BOM 组件价回退/禁静默0、选择列表 5 个/批分页、数量快捷条、移动审批 assignee 门禁；L71/L72）
...
| 2026-08-29 | **v4.4.4 PATCH：移动端「智能下单」对话式向导 + 零金额订单 BOM 子件计价修复 + 铁律11 文档及时更新**：①新增 `frontend-vue/src/views/mobile/MSmartOrder.vue`（路由 /mobile/smart-order，MHome 快捷入口首位），聊天气泡+选项点选为主的九步下单向导：订单类型(销售/补货/样品)→选客户(lookups/dealers，经销商账号自动锁定，补货校验寄售 consignmentEnabled)→搜产品(lookups/products 关键词)→数量→行折扣(仅销售)→是否加产品→整单优惠(仅销售：无/整单折扣/一口价/代金券)→折扣明细→确认(提交/保存草稿/取消)；选择题点选、仅关键词/数量/折扣值/样品原因走输入框，↺重新开始常驻+输入0重启+非法原地重问；金额全走 /api/sales-orders/preview，销售单取默认收货地址，补货/样品跳过全部折扣且样品必填原因单品；②后端 Critical 修复 L70：V4PriceEngine.expandBom 未透传母件 lineZero，BOM 子件(CHILD)在补货/样品/整单0 下仍按销售价计价（PRD-B001 补货 2 件 finalAmount=382.20 应为 0），加 parentLineZero 透参、子件随母件 setLineZero(true)；③AGENTS.md 新增【铁律11：文档及时更新】(§5.3.2 + §4.1 自检项)，layer1-rules §0.7 升级 + 铁律表第 10/11 条；验证：铁律9 八入口全过、域名形态 12/12、Playwright 智能下单全链路(销售折扣提交回读 PENDING_APPROVAL/草稿/代金券/一口价/补货 BOM ¥0.00/样品 ¥0.00/容错)、深度冒烟 272/272；CHANGELOG v4.4.4，无 Flyway/接口新增 | frontend-vue/src/views/mobile/MSmartOrder.vue + frontend-vue/src/router/index.js + frontend-vue/src/views/mobile/MHome.vue + backend/src/main/java/com/dms/v4/V4PriceEngine.java + AGENTS.md + .memory/layers/layer1-rules.md + .memory/layers/layer3-lessons.md(L70) + .memory/layers/layer4-decisions.md(D32/D33) + .memory/layers/layer5-context.md + docs/01_需求/DMS需求文档_汇总版.md + docs/02_设计/业务功能设计.md + CHANGELOG.md |
| 2026-08-30 | **v4.4.6 PATCH：移动端“很慢/页面刷不出来”修复（性能假象，实为鉴权续期）**：①主因：`frontend-vue/src/utils/request.js` 的 doRefresh() 在 access token 过期后误打 `/auth/refresh`，但系统 API 统一 `/api` 前缀、后端刷新接口是 `/api/auth/refresh`，Nginx 虽反代 `/auth/` 但后端无映射→404(No static resource auth/refresh)；token 约 18 分钟过期后业务请求 401→刷新 404→清登录态/请求挂起，表现为“搜产品转圈、页面刷不出”，而直连 API 都很快(80–145ms)、DB 无长查询、主机空闲，极易误判为性能问题；改打 `/api/auth/refresh` 并新增 redirectToLogin()（移动跳 /mobile/login、PC 跳 /login）；②次因：op_log varchar(255/16/8/64) 遇长 Java 方法签名/path/UA/remark 抛 22001 value too long 致整条日志 INSERT 失败（异步不阻塞但刷错）；OpLogEntry 加 @PrePersist enforceLengths() 落库前按列长 clip 截断；③诊断：主机 3.6G 内存偏紧(约380MB swap)但非主因，actuator/health ~1.4s 为 DB+Redis+MinIO 探针、前端不调用；验证：失效 access token 浏览器实测自动 POST /api/auth/refresh 拿新令牌并重放 /api/sales-orders 成功不掉登录、op_log 零 value too long、铁律9 八入口全过、移动冒烟 17/17；无 Flyway/接口/依赖变更 | frontend-vue/src/utils/request.js + backend/src/main/java/com/dms/operationlog/entity/OpLogEntry.java + .memory/layers/layer3-lessons.md(L73/L74) + layer4(D35) + layer5 + CHANGELOG.md |
| 2026-08-30 | **v4.4.7 PATCH：百分比折扣语义颠倒修复（98折被算成减98%）**：移动端智能下单第二行填行折扣 98（提示“9折填90”=98折=原价×0.98），后端 V4Money.signedDiscount() 对 PERCENT 一律 `amt=base×(value/100)` 当“减免比例”，导致填98=减98%只付2%（SO-20260830-00001 第2行 PRD-T006 4×6200=24800，lineDiscountAmount=-24304 应 -496）。修复：折扣方向(REDUCE) PERCENT 减免额=base×(1−factor)，factor=value>1?value/100:value（98→付98%减2%、90→减10%、100→不减）；加价(ADD)保持 factor 即加价比例。只影响手动行折扣+整单折扣（V4PriceEngine step4/step7，PC/移动同一后端一次修复两端）；产品全局折扣/客户全局折扣/促销满件打折走独立 rate(减免比例) 路径不受影响。移动端确认摘要把 98% 改为“9.8 折”消歧。验证：preview 行98折 lineDiscountTotal=560(=2%×28000)、行90折=2800(=10%)、100折=0、整单98折=548.8(=2%×27440)、固定金额折扣不受影响；真实浏览器智能下单 98 折摘要显示“行折扣：9.8 折”金额正确无红错；v446 回归 15/15、铁律9 八入口全过。历史已提交单为价格快照不重算；无 Flyway/接口变更 | backend/src/main/java/com/dms/v4/V4Money.java + frontend-vue/src/views/mobile/MSmartOrder.vue + .memory/layers/layer3-lessons.md(L75) + layer4(D36) + layer5 + docs/01_需求/DMS需求文档_汇总版.md(§36.3) + CHANGELOG.md |
| 2026-08-29 | **v4.4.5 PATCH：智能下单 4 项反馈修复**：①**BOM 组件价回退(L71, Critical)**：V4PricingService.salesPrice(BOM_COMPONENT) 未配 BOM 专属组件价时原静默 zeroPrice，销售单 BOM 子件（髓内钉 T005 的螺钉 T003/T004）金额全 0 整套器械近乎免费（SO-20260829-00003）；改为组件价链(DEALER/GLOBAL BOM_COMPONENT)未命中后回退单品销售价(STANDALONE)，仍无价抛 BusinessException（含产品编码+名称+母件），禁静默 0；母件行 finalAmount=0 是正确口径（价值由子件承载）；历史已提交单价格为快照不重算；②**移动审批越权门禁(L72)**：MApprovalDetail.vue myPendingTasks 原只按 status=PENDING 过滤，非指派人也看到同意/驳回→点击后端 403 only assignee can process task；引入 useUserStore，加 assigneeId 为空或等于 userStore.user.id 条件，与 PC ApprovalDetailDrawer 一致；③**选择列表分页**：MSmartOrder 客户/产品/代金券统一 PAGE_SIZE=5 + 上一批/下一批导航（onOption case page），编号全局连续、标题显示第 x/y 批，产品 limit 20→100；④**数量快捷条**：−/+ 步进 + 常用数量[1/5/10/20/50/100]点选 + 确定 N 件，与输入框双向同步；验证：v446_paging.js 15/15、BOM preview 子件 720+440=1160、sales(id32 非指派人)无按钮/admin(id1 指派人)有按钮、铁律9 八入口全过、深度冒烟 272/272；无 Flyway/接口新增 | backend/src/main/java/com/dms/v4/V4PricingService.java + frontend-vue/src/views/mobile/MSmartOrder.vue + frontend-vue/src/views/mobile/MApprovalDetail.vue + .memory/layers/layer3-lessons.md(L71/L72) + .memory/layers/layer5-context.md + docs/01_需求/DMS需求文档_汇总版.md(§36.3) + docs/02_设计/业务功能设计.md + CHANGELOG.md |
| 2026-08-30 | **v4.5.0 MINOR：厂家关闭进销存（开关持久化）+ 厂家↔经销商跨租户订单协同**：①厂家租户进销存菜单开关持久化回归修复——菜单过滤机制本就存在（layout 按 /api/tenant/features 的 inventoryEnabled/purchaseEnabled 隐藏 inventoryOnly/purchaseOnly，仅约束厂家用户），但 V7 种子写旧键 modules_enabled.inventory，新代码读不到即默认全开；用 **Flyway V140** 对所有 tenant_type=MANUFACTURER 置 inventoryEnabled/purchaseEnabled=false，经销商用户不受 TenantFeatureGuard 限制；②**跨租户协同**——路径A：经销商采购单供应商=平台厂家（suppliers.manufacturer_tenant_id）时，提交即在厂家租户自动生成 DRAFT 销售订单（对码经 product_mappings 转码，只带物料/数量/交期/备注+经销商采购单号行号，不带价格），厂家销售单号回写 vendor_order_code（列表新增「厂家销售单号」列）；路径B：厂家未经采购单直接出库给经销商时，出库回传经销商侧先自动生成 APPROVED 采购单再生成待收货入库单；出库→收货回传两路径共用（产品转经销商编码、数量1:1、批次/序列号、不带价格）；对码缺失阻断并提示缺哪个产品编码；台账 cross_tenant_doc_links（line_refs JSON 存行级批次/序列号，sales_out_id/po_id 唯一约束幂等）；新增字段 suppliers.manufacturer_tenant_id、purchase_orders.vendor_order_code、orders.customer_po_code；V140 同时为现有经销商租户自动补建「平台厂家」系统供应商；③新增 com.dms.collab.CrossTenantCollabService（onPurchaseOrderSubmitted/onSalesOutShipped）挂钩 PurchaseOrderService.submit 与 SalesOutService.partialShip，跨租户写入靠切换 TenantContext + 显式 tenantId 原生 SQL；本期不做取消/红冲跨租户联动、不传价格折扣、不共享库存、换算率固定 1:1；验证：路径A（PO→自动 SO→补价提交→部分出库→自动待收货 RK 单）、路径B（直接出库→自动已审批 PO+RK 单）、对码缺失 40006 阻断回滚、幂等台账各 1 行、铁律9 八入口全过；仅测试环境 | backend/src/main/java/com/dms/collab/** + backend/src/main/java/com/dms/purchase/service/PurchaseOrderService.java + backend/src/main/java/com/dms/sales/service/SalesOutService.java + backend/src/main/resources/db/migration/V140__v450_cross_tenant_collab.sql + frontend-vue/src/views/PurchaseOrderList.vue(vendor_order_code 列) + docs/03_接口文档/cross-tenant-collab-api.md + docs/03_测试/测试报告_v4.5.0_20260830_跨租户协同.md + CHANGELOG.md |
| 2026-08-31 | **v4.5.1 PATCH：跨租户协同自动化回归补齐 + 分批/幂等/幽灵库存缺陷修复**：v4.5.0 首版仅手工测试，本次补齐双层自动化回归并修 4 个链路缺陷 + 1 个测试基建缺陷。①**二次部分发货整笔跳过(High)**：路径B 幂等只按出库单 ID 查台账，首张出库单回传后同销售订单的第二张出库单所有行被误判"已回传"；改为 outLineId 行级粒度去重（从台账 line_refs 收集 doneOutLineIds 只过滤真正回传过的行）；②**分批重复补 PO(High)**：未命中路径A台账时每次发货都新建自动 PO；统一补建/复用——先 linkRowBySalesOrder（按销售订单链路）再 linkRowBySalesOut（历史出库台账）查已有自动 PO，命中则 upsertPoLineQty 累加数量（5+3=8），无则才补建；③**upsertPoLineQty SQL 无效列(Medium)**：purchase_order_lines 无 updated_at（仅 created_at），INSERT 带该列分批累加抛错，删除；④**路径B 收货单缺 dealer_id/warehouse_id（幽灵库存,High）**：新增 resolveOrCreateDealerSelf（自动补建 DEALER-SELF/本企业主体）与 resolveOrCreateDefaultWarehouse（COLLAB-DEFAULT-WH/main 默认仓），收货单与后续入库落正确主体/仓库；⑤**测试基建撞 V136 NOT NULL**：V136 给 dealers 加 consignment_enabled/consignment_limit/credit_limit/payment_days 四个 NOT NULL 列（有 DEFAULT），但 Hibernate 对未赋值字段显式插 null、DEFAULT 不生效；BaseIntegrationTest.createTestDealer 工厂 + SalesReturnLockingIntegrationTest(2处) + SalesOrderApprovalOutboundChainTest(4处) 直接 Dealer.builder() 造数全部补齐 false/0/0/0；⑥**新增双层自动化**：automation_test/v450_collab.py（接口+DB 回读，TAG 种子自清理，覆盖 10 缺口）56/56 通过；CrossTenantCollabIntegrationTest（zonky 内嵌 PG14 + Flyway 全量，TenantContext 切双租户直调 service，路径A 5 例+路径B 6 例）11/11 通过，既有 Inventory 7/Order 7/SalesReturn 2/Chain 4 共 20 例回归全绿，合计 31/31；⑦Flyway V141（演示数据迁移补回本地代码库）、V142（collab 部分发货多链路索引，ux 唯一索引改 ix 非唯一，支持同销售订单多张出库单回传）；修复已部署测试环境 stamp 20260831-122753 | backend/src/main/java/com/dms/collab/CrossTenantCollabService.java + backend/src/test/java/com/dms/BaseIntegrationTest.java + backend/src/test/java/com/dms/collab/CrossTenantCollabIntegrationTest.java（新）+ backend/src/test/java/com/dms/order/SalesReturnLockingIntegrationTest.java + backend/src/test/java/com/dms/chain/SalesOrderApprovalOutboundChainTest.java + backend/src/main/resources/db/migration/V141__*.sql + V142__*.sql + automation_test/v450_collab.py（新）+ docs/03_测试/测试报告_v4.5.0_20260830_跨租户协同.md(§8) + CHANGELOG.md |
| 2026-08-31 | **v4.5.2 PATCH：接入 GitHub Actions CI 流水线 + 全量测试基线修复**：①**CI 工作流**新建 `.github/workflows/ci.yml`——push/PR 到 main 触发，两个并行 job：`backend-test`（ubuntu + temurin JDK17 + setup-java cache:maven，`working-directory: backend` 跑 `mvn -B verify`，失败也上传 surefire-reports artifact）、`frontend-build`（Node 20 + setup-node cache:npm（cache-dependency-path: frontend-vue/package-lock.json），`npm ci` → `npm test`(vitest run) → `npm run build`(vite)，失败也上传 dist artifact）；CI 不需要 Docker/PG/Redis——后端测试走 zonky embedded-postgres 内嵌 PG14（EmbeddedPostgresSessionListener SPI），Redis @MockBean；pom 仓库全为 aliyun 镜像，海外 runner 可访问；②**全量测试基线修复 12 例**（此前只跑 `-Dtest` 子集未暴露）：11 例旧 Controller 集成测试（Product/Promotion/ProductBundle 等）造用户后未授予权限，@PreAuthorize 精确匹配 GrantedAuthority 全返回 403——补 `grantPermissions` 显式权限码（如 product:view/search/create/edit）；注意 `grantPermissions(user,"*")` 的 `"*"` 是**字面资源码不是通配**（PermissionQueryService 不展开），且 CoreDomainEndpointTest 断言 `isNotIn(404,5xx)` 不含 403 会假性通过；1 例 V4CalculatorTest 百分比折扣造数过时——v4.4.7 起 PERCENT 为中文"折数"语义（填 90=9折=付90%/减10%），测试填 20 实为减80%（折扣额 800 ≠ 期望 200），改为填 80（8折=减20%=折扣额 200，下游断言全部不变）；③**验证**：本地 `mvn -B test` → Tests run 147, Failures 0, Errors 0；本机无 node，经 plink 在测试服务器（node v20.20.2，与 CI Node20 等价）实测 `npm ci`(299 包) → vitest 4 文件 36 例全过 → `vite build` 成功（28.51s，2957 模块）；④CI 激活前提：工作流文件需 git commit + push 到 GitHub（`git@github.com:vinkinyyx/dms-demo.git` main 分支）才会触发，本次按用户要求**不部署生产**；新增踩坑点 55-57；无 Flyway/接口/生产代码逻辑变更（仅测试代码 + CI 配置） | .github/workflows/ci.yml（新）+ backend/src/test/java/com/dms/masterdata/controller/ProductControllerIntegrationTest.java + ProductBundleControllerIntegrationTest.java + promotion/controller/PromotionControllerIntegrationTest.java + backend/src/test/java/com/dms/v4/V4CalculatorTest.java + CHANGELOG.md + docs/AI开发文档.md + doc/AI开发文档.md |
| 2026-08-31 | **v4.5.3 PATCH（PC 工作台 UI 增强，纯前端）：面包屑导航 + 多菜单页签 Tags-View**。参考主流后台（HTDK）在内容区顶部增加两大导航能力，用户无需反复打开左侧菜单即可定位/切换页面。①**面包屑 Breadcrumb.vue**：el-breadcrumb（separator="/"）展示「首页 / 菜单分组 / 菜单 / 当前页」完整路径，中间层级可点击回跳、末级当前页纯文本；②**多页签 TagsBar.vue**：每访问一个菜单开一个页签 chip，支持点击切换、×关闭、鼠标中键关闭、右键菜单（刷新页面/关闭页签/关闭其他/关闭全部，固定首页"关闭页签"置灰），激活页签自动横向滚动居中，配合 `<keep-alive :max="12">` 缓存页面状态，"刷新页面"通过 reloading 信号销毁重建当前页；页签用 **sessionStorage（key `dms:tags-view`）持久化，F5 后恢复**，首页页签 affix 固定不可关，退出登录 reset 清场；③**pageMeta.js 元信息解析器**：由 config/menu.js 的 MENU_GROUPS 扁平化匹配（/home 特判 → fullPath/path 精确 → 前缀 → 27 条 DETAIL_PARENT 正则映射详情页父菜单 → 兜底），tagKey 菜单页用 fullPath（报表 query 区分）、非菜单页用 name；④样式全用 DMS CSS 变量，深色模式 `:global(html[data-mode='dark'])` 适配。验证：本地 vitest 36/36 + vite build（VITE_BASE=/dms/）；铁律9 真实浏览器逐项过（面包屑三级/详情页父级、页签增/切/关/右键四功能/关闭其他/关闭全部/F5 恢复/深色模式/面包屑链接回跳），Console 无 error，必检 URL（/、/dms/、/dms/home、/dms/admin/、mobile/login、mobile/register）全 200；已部署测试环境（备份 stamp 20260831-165944，纯静态解压 + nginx reload，未动 admin 与后端）；新增踩坑点 58-61；无 Flyway/接口变更 | frontend-vue/src/layout/Breadcrumb.vue（新）+ frontend-vue/src/layout/TagsBar.vue（新）+ frontend-vue/src/utils/pageMeta.js（新）+ frontend-vue/src/store/tags.js（新）+ frontend-vue/src/layout/index.vue + CHANGELOG.md + docs/AI开发文档.md + doc/AI开发文档.md + docs/项目设计文档.md + doc/项目设计文档.md |
...
52. **【v4.5.1 教训】Flyway 给已有表加 NOT NULL 列（带 DEFAULT）后，JPA/Hibernate 造数必须显式赋值**：V136 给 dealers 加了 `consignment_enabled boolean NOT NULL DEFAULT false`、`consignment_limit/credit_limit numeric(18,2) NOT NULL DEFAULT 0`、`payment_days integer NOT NULL DEFAULT 0`。DDL 的 DEFAULT **只对未出现在 INSERT 列表中的列生效**；而 Hibernate 对实体中未赋值的 Boolean/BigDecimal/Integer 字段会生成**显式 NULL 的 INSERT**，DEFAULT 不触发，直接 DataIntegrityViolation（列名 NOT NULL 违例）。集成测试里凡是 `Dealer.builder()...build()` 造数（含 BaseIntegrationTest.createTestDealer 工厂和绕过工厂直 builder 的测试类）都必须显式 `.consignmentEnabled(false).consignmentLimit(BigDecimal.ZERO).creditLimit(BigDecimal.ZERO).paymentDays(0)`。规律：**任何 Flyway 新增 NOT NULL 列，都要全量 grep 测试代码里对应实体的 `.builder()`/`new Xxx(...)` 造数点补齐默认值**，否则整个测试基建崩。
53. **【v4.5.1 教训】跨租户/跨单据自动联动的幂等键必须是"行级"，不能按单据 ID 粗粒度去重；补单逻辑要先复用后新建**：路径B（厂家出库→经销商自动 PO+收货）最初只按出库单 ID 查 cross_tenant_doc_links 台账判断"回传过没有"，结果同一张销售订单分两次部分发货（两张出库单）时，第二张出库单的所有行被误判为已回传而整笔跳过；且未命中路径A台账时每次发货都新建一张自动 PO（重复单）。正确做法：①幂等粒度落到 **outLineId 行级**——从台账 line_refs JSON 收集 doneOutLineIds，只过滤真正回传过的明细行，未回传行照常处理；②补建自动单据前先按业务链路（销售订单→PO）再按历史台账（出库→PO）两级查找已有自动单据，命中则 upsert 累加数量（5+3=8），无则才新建；③自动补建的下游单据必须带全业务外键（收货单缺 dealer_id/warehouse_id 会产生"幽灵库存"——库存挂在空主体/空仓下），必要时自动补建系统主体（DEALER-SELF）与默认仓（COLLAB-DEFAULT-WH）。
54. **【v4.5.1 测试基建】后端集成测试范式（embedded-postgres）**：测试基类 `BaseIntegrationTest`（@SpringBootTest + @Transactional 每方法回滚），zonky embedded-postgres 2.0.7 起**内嵌 PostgreSQL 14.10** + Flyway 全量迁移建表，RedissonClient/MinioStorageService 用 @MockBean；跨租户逻辑用 TenantContext ThreadLocal 切租户 + applicationContext.getBean() 直调 Service 层（绕过 HTTP），原生 SQL 助手用 `em.createNativeQuery` 回读断言。运行：`$env:JAVA_HOME="C:\tools\jdk-17.0.13+11"; cd backend; mvn -Dtest=类名 -DfailIfNoTests=false test`；**`-q` 会抑制 "Tests run:" 汇总**，结果直接读 `backend\target\surefire-reports\*.txt`；多个测试类放同一 `-Dtest=A,B,C` 可复用同一 Spring 上下文与内嵌 PG（首个类启动约 100s+，后续类秒级）。注意 DocNoGenerator.next() 是 REQUIRES_NEW 独立事务，序列行不随测试回滚（无害）。
55. **【v4.5.2 CI 教训】全量 `mvn test` 与 `-Dtest=子集` 结果可能完全不同——CI/交付前必须跑全量**：日常开发只用 `-Dtest=当前类` 跑相关测试，上下文里 Spring Security 可能因其他配置放行；而全量 `mvn test`/`mvn verify` 会逐个加载所有测试类，旧 Controller 集成测试（早期权限体系未成型时写的）造用户后不授权，@PreAuthorize("@perm.hasAny('xxx:view')") 精确匹配 GrantedAuthority 全部返回 403。两个隐蔽点：①`grantPermissions(user, "*")` 的 `"*"` 是**字面资源码，不是通配符**——PermissionQueryService 不会把它展开为全部权限，授予后用户仍无任何具体权限；②`CoreDomainEndpointTest` 断言用 `isNotIn(404, 5xx)`，**不含 403**，所以权限缺失导致的 403 被当成"通过"假性全绿。规律：新写 Controller 测试必须用 `grantPermissions(user, "product:view", "product:search", ...)` 授予与接口 @PreAuthorize **完全一致的权限码**；断言集合要显式包含 403 反向用例（无权限用户应被拒）。
56. **【v4.5.2 测试造数教训】业务语义变更后，旧测试的"魔法数字"造数必须同步重算，不能只改生产代码**：v4.4.7 把 PERCENT 折扣从"减免比例"改为中文"折数"语义（V4Money.signedDiscount：value>1 时 factor=value/100 为实付比例，REDUCE 减免额=base×(1−factor)；填 90=9折=减10%、填 80=减20%、填 20=减80%），生产代码与前端提示（"9折填90"）一致，但 V4CalculatorTest 仍按旧语义填 20 期望减免 200——实际减免 800。修复时不是改期望值迁就错误输出，而是按新语义反推造数：要让折扣额=200（=base×20%），折数应填 **80**，下游所有金额断言（166.15/103.85/633.85/396.15/合计 1030）恰好全部不变。规律：涉及计价/分摊/税率等计算逻辑的语义修复，必须 grep 测试代码里对应枚举值的造数点（`PERCENT`/`RATE` + BigDecimal），按新语义重算输入值；前端与后端同一套语义时以后端实现+前端提示文案交叉验证哪边正确。
57. **【v4.5.2 环境教训】本机无 Node 时可用测试服务器等价模拟 CI；Windows zip 传 Linux 丢权限位、plink 后台任务需 setsid**：①CI 的 Node 20 链路在本地 Windows 无 node/npm 无法验证时，经 plink 连测试服务器 43.128.145.141（自带 node v20.20.2/npm 10.8.2，与 GitHub Actions ubuntu runner 的 Node 20 等价）跑 `npm ci && npm test && npm run build` 即可提前暴露问题；②**Windows `Compress-Archive` 打的 zip 传到 Linux 解压后目录权限位丢失**（vitest 报 `EACCES: scandir '.../src/assets/brand'`），解压后先 `chmod -R u+rwX,go-rwx src public` 再跑；③**plink 执行 `nohup ... &` 后台任务会随 SSH 断开被杀**（日志文件都不生成），必须用 `setsid bash -c '...' < /dev/null > /tmp/x.log 2>&1 &` 脱离会话；④plink batch 模式首次连接报 host key 未缓存，加 `-hostkey "SHA256:v5BiatnUZbRj81JP0twQByG1yogK7rVGkRcmBRWEHEU"`（测试服务器 ssh-ed25519 指纹）。
58. **【v4.5.3 前端构建】业务前端必须 `VITE_BASE=/dms/` 构建；本机无系统 node 时用便携版 Node**：①vite.config.js 是 `base: process.env.VITE_BASE || '/'`，直接 `npm run build` 默认 base=/，产物 index.html 引用 `/assets/index-*.js`，部署到测试服（业务前端挂在 `/dms/` 子路径）后资源 404；必须 PowerShell 里 `$env:VITE_BASE="/dms/"; npm run build`，产物引用 `/dms/assets/index-*.js` 才正确。admin-vue 是**独立工程**，用 `VITE_BASE=/dms/admin/` 构建，不要与业务前端混。②本机系统 PATH 无 node/npm 时，便携版 Node 在 `C:\Users\<用户>\AppData\Local\node-portable\node-v22.11.0-win-x64`（node v22.11.0/npm 10.9.0），每条命令前 `$env:PATH = "<该路径>;$env:PATH"`；node_modules 缺失先 `npm install --no-audit --no-fund`。
59. **【v4.5.3 部署】前端纯静态更新无需重建镜像/force-recreate，nginx reload 即可**：测试服 dms-test-nginx 容器 bind-mount `./frontend:/usr/share/nginx/html:ro`，业务前端静态文件在 `frontend/dms/`、独立 admin 在 `frontend/dms/admin/`、根 `/` 是宣传页。只改业务前端时的定式：备份 `frontend` → `find frontend/dms -mindepth 1 -maxdepth 1 ! -name admin -exec rm -rf`（**保留 dms/admin**）→ 解压新 dist 到 `frontend/dms/` → `chown -R root:root` → `docker exec dms-test-nginx nginx -t && docker exec dms-test-nginx nginx -s reload` → curl/浏览器验证。**不要** `docker compose up --force-recreate`（那会连带重启 backend）。部署后必须 grep 容器内/静态产物关键标记（如 bundle 里含 `dms:tags-view`、`app-breadcrumb`）确认是新代码，不能只看源码目录。
60. **【v4.5.3 前端路由】/m/:key 模块页的 key 必须与 config/menu.js 的 `key` 逐字一致，页签/面包屑元信息靠它解析**：模块路由是 `/m/:moduleKey` → ModuleView 动态渲染，pageMeta.js 用 MENU_GROUPS 扁平化后的 route（`/m/` + key）匹配菜单标题与面包屑。测试/直连 URL 时若写错 key（如产品分类菜单 key 是 `categories`，误写成 `/m/product-categories`），匹配不到菜单会走兜底，页签标题显示"业务模块"、面包屑只有"首页"——这是 URL 用错，不是组件缺陷。验证页签/面包屑时先 grep menu.js 确认真实 key；详情页（如 `/dealers/1`）靠 pageMeta.js 的 DETAIL_PARENT 正则映射回父菜单（经销商管理），新增详情页路由要补对应正则。
61. **【v4.5.3 浏览器自动化】非语义 div 组件（页签 chip）在 AX 快照里无 interactive ref，用 browser_evaluate 原生事件驱动**：TagsBar 的页签 chip 是带 @click 的普通 div（无 role/button 语义），accessibility 快照里只表现为一个文本节点（如 `text "工作台首页 产品管理"`），拿不到可点击 ref。验证这类组件用 `browser_evaluate` 注入原生 JS：点击用 `el.click()`、关闭按钮用 `el.querySelector('.el-icon,svg').dispatchEvent(new MouseEvent('click',{bubbles:true}))`、右键菜单用 `el.dispatchEvent(new MouseEvent('contextmenu',{bubbles:true,cancelable:true,clientX,clientY}))`（clientX/Y 用 getBoundingClientRect 算，否则菜单定位到 0,0）。读计算样式验证深色模式用 `getComputedStyle(el)`；注意验证边框要读对应方位（border-bottom-color 而非 border-top-color）。
