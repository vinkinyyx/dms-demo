  ...
  <div class="login-logs-page">
    <el-card shadow="never">
      <div class="toolbar">
        ...
        <el-date-picker v-model="dateRange" type="datetimerange" value-format="YYYY-MM-DDTHH:mm:ss" start-placeholder="开始时间" end-placeholder="结束时间" />
      ...
      <el-table :data="rows" v-loading="loading" border stripe>
        ...
        <el-table-column prop="loginType" label="类型" width="110" />
        <el-table-column label="结果" width="90">
          <template #default="{ row }"><el-tag :type="row.success ? 'success' : 'danger'">{{ row.successLabel || (row.success ? '成功' : '失败') }}</el-tag></template>
        </el-table-column>
...
async function reload(page=query.page){
  ...
  try{
    ...
    const r=await request({url:'/api/system/login-logs',method:'get',params})
    rows.value=r.data?.list||[]; total.value=r.data?.total||0
...
<style scoped>
.toolbar{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px}.pager{margin-top:12px;justify-content:flex-end;display:flex}
</style>
