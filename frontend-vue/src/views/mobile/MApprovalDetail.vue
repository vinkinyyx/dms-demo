...
const route=useRoute(); const detail=ref(null); const summary=ref({}); const comment=ref(''); const commentVisible=ref(false); let action=''
...
async function load(){ const id=route.params.id; const [d,s]=await Promise.all([request({url:'/api/approval/instances/'+id}), request({url:'/api/approval/instances/'+id+'/summary'})]); detail.value=d.data; summary.value=s.data||{} }
