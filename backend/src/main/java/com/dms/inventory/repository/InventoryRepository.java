...
public interface InventoryRepository extends JpaRepository<Inventory, Long> {
    ...
    @Query(value = "SELECT * FROM inventory " +
            ...
            "   AND NULLIF(batch_no, '') IS NOT DISTINCT FROM NULLIF(:batchNo, '') " +
            "   AND NULLIF(serial_no, '') IS NOT DISTINCT FROM NULLIF(:serialNo, '') " +
            " ORDER BY qty DESC, updated_at DESC, id DESC LIMIT 1", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM inventory " +
            ...
            "   AND NULLIF(batch_no, '') IS NOT DISTINCT FROM NULLIF(:batchNo, '') " +
            "   AND NULLIF(serial_no, '') IS NOT DISTINCT FROM NULLIF(:serialNo, '') " +
            " ORDER BY qty DESC, updated_at DESC, id DESC LIMIT 1 FOR UPDATE", nativeQuery = true)
