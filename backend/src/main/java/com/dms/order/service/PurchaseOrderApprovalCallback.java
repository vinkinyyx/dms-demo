...
public class PurchaseOrderApprovalCallback implements ApprovalBusinessCallback {
    ...
    public void onApproved(ApprovalInstance instance) {
        em.createNativeQuery("UPDATE purchase_orders SET status='APPROVED', approved_at=now(), updated_at=now() WHERE id=?1 AND tenant_id=?2")
                .setParameter(1, instance.getBusinessId()).setParameter(2, instance.getTenantId()).executeUpdate();
        try {
            Long receiptId = autoDocGenerator.createReceiptForPurchaseOrder(instance.getBusinessId());
            log.info("purchase order {} approved, auto receipt {}", instance.getBusinessId(), receiptId);
        } catch (Exception e) {
            log.warn("purchase order {} create receipt failed: {}", instance.getBusinessId(), e.getMessage());
        }
    }
    ...
    public void onRejected(ApprovalInstance instance) {
        em.createNativeQuery("UPDATE purchase_orders SET status='REJECTED', updated_at=now() WHERE id=?1 AND tenant_id=?2")
                .setParameter(1, instance.getBusinessId()).setParameter(2, instance.getTenantId()).executeUpdate();
    }
    ...
    public void onTerminated(ApprovalInstance instance, String result) {
        ...
        else em.createNativeQuery("UPDATE purchase_orders SET status='CANCELLED', updated_at=now() WHERE id=?1 AND tenant_id=?2")
                .setParameter(1, instance.getBusinessId()).setParameter(2, instance.getTenantId()).executeUpdate();
    }
