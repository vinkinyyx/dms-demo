...
public class ProductCreateApprovalCallback implements ApprovalBusinessCallback {
    public static final String BUSINESS_TYPE = "PRODUCT_CREATE";
    ...
    @Override public void onApproved(ApprovalInstance instance) {
        em.createNativeQuery("UPDATE products SET status='active', updated_at=now() WHERE id=?1 AND tenant_id=?2")
          .setParameter(1, instance.getBusinessId()).setParameter(2, instance.getTenantId()).executeUpdate();
        writeOpLog(instance, "APPROVE", "产品创建-审批通过");
    }
    @Override public void onRejected(ApprovalInstance instance) {
        em.createNativeQuery("UPDATE products SET status='draft', updated_at=now() WHERE id=?1 AND tenant_id=?2")
          .setParameter(1, instance.getBusinessId()).setParameter(2, instance.getTenantId()).executeUpdate();
        writeOpLog(instance, "REJECT", "产品创建-审批驳回");
    }
    @Override public void onReturned(ApprovalInstance instance) {
        em.createNativeQuery("UPDATE products SET status='draft', updated_at=now() WHERE id=?1 AND tenant_id=?2")
          .setParameter(1, instance.getBusinessId()).setParameter(2, instance.getTenantId()).executeUpdate();
        writeOpLog(instance, "UPDATE", "产品创建-审批退回");
    }
    @Override public void onWithdrawn(ApprovalInstance instance) {
        em.createNativeQuery("UPDATE products SET status='draft', updated_at=now() WHERE id=?1 AND tenant_id=?2")
          .setParameter(1, instance.getBusinessId()).setParameter(2, instance.getTenantId()).executeUpdate();
        writeOpLog(instance, "UPDATE", "产品创建-撤回审批");
    }
    @Override public void onTerminated(ApprovalInstance instance, String result) {
        em.createNativeQuery("UPDATE products SET status='draft', updated_at=now() WHERE id=?1 AND tenant_id=?2")
          .setParameter(1, instance.getBusinessId()).setParameter(2, instance.getTenantId()).executeUpdate();
        writeOpLog(instance, "UPDATE", "产品创建-审批终止");
    }
