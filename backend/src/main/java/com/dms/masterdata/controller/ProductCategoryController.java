...
public class ProductCategoryController {
    ...
    @PutMapping("/{id}")
    @OperationLog(businessType = "productCategory", action = OperationAction.UPDATE, remark = "产品分类-更新")
    ...
    @DeleteMapping("/{id}")
    @OperationLog(businessType = "productCategory", action = OperationAction.DELETE, remark = "产品分类-删除")
    ...
    public ResponseEntity<byte[]> export() throws Exception {
        ...
        pq.setPage(1);
        pq.setSize(10000);
        java.util.List<ProductCategory> list = service.list(pq, null).getList();
        ...
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION, ContentDispositionUtils.attachment("产品分类列表.xlsx"))
    ...
    public ResponseEntity<byte[]> exportTemplate() throws Exception {
        String[] headers = {"编码", "名称", "父级ID", "级别", "排序", "状态"};
        String[] fieldNames = {"code", "name", "parentId", "level", "sortOrder", "status"};
        String[] examples = {"CAT-001", "示例分类", "0", "1", "1", "active"};

        byte[] excelBytes = ExcelExportUtils.exportTemplate(headers, fieldNames, examples);

        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=product-categories_template.xlsx")
                .contentType(MediaType.APPLICATION_OCTET_STREAM)
                .body(excelBytes);
    }

    @PostMapping("/batch-import")
    @OperationLog(businessType = "productCategory", action = OperationAction.CREATE, remark = "产品分类-批量导入")
