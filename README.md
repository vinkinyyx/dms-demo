...
- 子件下单价格只取该 BOM 上下文的 BOM_COMPONENT 价（查不到返回 0，不回退单品价）；修复子件行折扣丢失，行折扣优先级最高（先行折扣，再分摊整单折扣/促销满减）。
...
- 新增 V4 计价引擎与价格/BOM/促销模型（Flyway V96-V107），支持 BOM 母件-子件、行级/整单折扣、促销满减与赠品、价格生效/失效与多上下文隔离。
- 订单行折扣与金额占比分摊、促销赠品手动刷新与幂等、标准金额占比分摊等口径统一；产品/价格/订单等核心模块的只读字段全部显示业务名称而非裸 ID。
- 新增租户能力守卫（TenantFeatureGuard/TenantFeatureAspect）、统一 Jackson 配置，以及 utomation_test/e2e/ 下分模块 E2E 规格与 v4 API 回归脚本。
...
- 数据：新增 16 个专属销售账号分散到各岗位，sys_admin 可见全部手术报台；手术报台按销售数据权限可见。
- 生产环境统一走 80 端口网关，DMS 挂在 `/dms/` 子路径；会话有效期显式化为 8 小时。
- 测试环境通过关键接口 smoke 与 E2E 验证。
## v3.10.0（2026-08-09）- 审批流引擎与审批中心

- 新增审批模块：模板版本、条件匹配、顺序节点、账号/角色审批人、ANY/ALL 多人规则、前/后加签、转办、全局委托、抄送、管理员改派/终止、待办与审批记录。
- 销售订单、采购订单提交进入审批（PENDING_APPROVAL），通过后自动生成出库/入库草稿；未配置模板时默认自动通过。
...
- 部署：测试环境 43.128.145.141 已上线，端到端 API 全部 200。
...
### 正式环境（生产）
v3.12.4 起生产环境统一走 80 端口网关，DMS 挂在 `/dms/` 子路径（`/` 仍为产品宣传手册，`/ai/` 为 ai-knowledge），API/健康检查走根路径反代到后端容器。
| 用途 | URL / 命令 |
|---|---|
...
- 正式环境统一 80 端口网关（webgate/nginx），DMS 挂 `/dms/` 子路径，后端仅监听 127.0.0.1:18080
- 测试环境统一 80 端口，后端不对外暴露
- 数据库/Redis/MinIO 均仅容器内网访问，不对外暴露端口
